...
import main.scala.overlapping.containers.{SingleAxisBlock, TSInstant}
import org.apache.spark.rdd.RDD
import org.apache.spark.{SparkConf, SparkContext}
import org.scalatest.{FlatSpec, Matchers}


class TestVARModelSparse extends FlatSpec with Matchers{

  "The VAR model " should " retrieve VAR parameters with sparse Bayesian estimation." in {
    ...
    val estimARCoeffs = VARL1GradientDescent(overlappingRDD, p, 1e-2)

    for(i <- 0 until p) {
      for(j <- 0 until d) {

        for(k <- 0 until d){
          estimARCoeffs(i)(j, k) should be (ARcoeffs(i)(j, k) +- 0.10)
        }
